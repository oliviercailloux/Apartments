...
public class ApartmentStatistics {
  ...
  /**
   * Give a list of apartments on which we will work, his length is given by the user. In the
   * future, we would want to sort all apartments according to their value function of each
   * apartment. So that, the number would take the best apartments of the sorted list.
   * 
  ...
  /**
   * Load numeric data of the given criteria and create a Stats object in order to get statistics of
   * our apartments.
   * 
  ...
  public Stats getNumericStatistics(Criterion featureName) {
    ...
    switch (featureName) {
      ...
      case FLOOR_AREA_TERRACE:
        ...
        for (int i = 0; i < myApartments.size(); i++) {
          floorAreaTerraceStats
              .add(Math.round(myApartments.get(i).getFloorAreaTerrace() * 100.0) / 100.0);
        }
      ...
      case PRICE_PER_NIGHT: {
        ...
        for (int i = 0; i < myApartments.size(); i++) {
          pricePerNightStats
              .add(Math.round(myApartments.get(i).getPricePerNight() * 100.0) / 100.0);
        }
  ...
  /**
   * Load boolean data of the given criteria and create an HashMap (size 2) that has as keys true
   * and false and as values their respective occurrences for the given apartments.
   * 
   * @param the criterion that we want to extract statistics.
