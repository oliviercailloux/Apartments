package io.github.oliviercailloux.apartments.apartment;
...
public class ApartmentStatistics {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Apartment.class);

	private List<Apartment> myApartments;

	public static ApartmentStatistics given(List<Apartment> listApartments,
			Integer number) {
		return new ApartmentStatistics(listApartments, number);
	}

	private ApartmentStatistics(List<Apartment> listApartments,
			Integer number) {
		this.myApartments = getApartmentsSample(listApartments, number);
	}

	/**
	 * Give a list of apartments on which we will work, his length is given by
	 * the user. In the future, we would want to sort all apartments according
	 * to their value function of each apartment. So that, the number would take
	 * the best apartments of the sorted list.
	 * 
	 * @param a
	 *            larger list of apartments and the number that we want to study
	 * @return a sample of apartments
	 */

	private List<Apartment> getApartmentsSample(List<Apartment> listApartments,
			Integer number) {
	...
	/**
	 * Load numeric data of the given criteria and create a Stats object in
	 * order to get statistics of our apartments.
	 * 
	 * More information on Stats :
	 * https://guava.dev/releases/23.0/api/docs/com/google/common/math/Stats.html
	 * 
	 * @param the
	 *            criterion that we want to extract statistics.
		...
		switch (featureName) {
			...
			case FLOOR_AREA :
				...
				for (int i = 0; i < myApartments.size(); i++) {
					floorAreaStats.add(Math
							.round(myApartments.get(i).getFloorArea() * 100.0)
							/ 100.0);
				}
			...
			case FLOOR_AREA_TERRACE :
				...
				for (int i = 0; i < myApartments.size(); i++) {
					floorAreaTerraceStats.add(Math.round(
							myApartments.get(i).getFloorAreaTerrace() * 100.0)
							/ 100.0);
				}
			...
			case NB_BATHROOMS : {
				...
				for (int i = 0; i < myApartments.size(); i++) {
					nbBathroomsStats.add(Double
							.valueOf(myApartments.get(i).getNbBathrooms()));
				}
			...
			case NB_BEDROOMS : {
				...
				for (int i = 0; i < myApartments.size(); i++) {
					nbBedroomsStats.add(Double
							.valueOf(myApartments.get(i).getNbBedrooms()));
				}
			...
			case NB_MIN_NIGHT : {
				...
				for (int i = 0; i < myApartments.size(); i++) {
					nbMinNightStats.add(Double
							.valueOf(myApartments.get(i).getNbMinNight()));
				}
				LOGGER.info(
						"The requested minimum number of night statistics are available");
			...
			case NB_SLEEPING : {
				...
				for (int i = 0; i < myApartments.size(); i++) {
					nbSleepingStats.add(Double
							.valueOf(myApartments.get(i).getNbSleeping()));
				}
			...
			case PRICE_PER_NIGHT : {
				...
				for (int i = 0; i < myApartments.size(); i++) {
					pricePerNightStats.add(Math.round(
							myApartments.get(i).getPricePerNight() * 100.0)
							/ 100.0);
				}
			...
			default :
				throw new IllegalArgumentException(
						featureName + " isn't a numeric Apartment feature");
		}

	}

	/**
	 * Load boolean data of the given criteria and create an HashMap (size 2)
	 * that has as keys true and false and as values their respective
	 * occurrences for the given apartments.
	 * 
	 * @param the
	 *            criterion that we want to extract statistics.
	...
	public Map<Boolean, Integer> getBooleanStatistics(Criterion featureName) {
		...
		switch (featureName) {
			...
			default :
				throw new IllegalArgumentException(
						featureName + " isn't a boolean Apartment feature");
		}
