package io.github.oliviercailloux.apartments.plot;
...
public class Histograms {
      ...
      case FLOOR_AREA_TERRACE:
        ...
        for (int i = 0; i < apartments.size(); i++) {
          floorAreaTerraceStats
              .add(Math.round(apartments.get(i).getFloorAreaTerrace() * 100.0) / 100.0);
        }
  ...
  public static JFreeChart createHistogram(ArrayList<Double> dataList) {
    ...
    JFreeChart histogram = ChartFactory.createHistogram(crit + " statistics", crit.toString(),
        "Number of apartments", dataset);
    return histogram;
  }
  ...
  /**
   * Enables to change the criteria and its linked histogram with a method that works like the given
   * (but more usable from the user point of the view).
   */
