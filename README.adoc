...
* Update the `idea.adoc` by regrouping your FUTURE mentions. Moreover, Iin case we can't merge the SpiderPlot into the master branch, we would like to create a documentation for the future students of the projet in order to make them able to carry this work on. This task can be relevant because some diagrams in master include SpiderPlot notions.
